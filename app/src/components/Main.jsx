...
export default function Main({ network, voteAccount }) {
  ...
  useEffect(() => {
    ...
    async function getVotes() {
      ...
      try {
        ...
        setVotes({
          tea: parseInt(account.tea.toString()),
          coffee: parseInt(account.coffee.toString()),
        });
      } catch (error) {
  ...
  // Vote for either tea or coffee. Poll for updated vote count on completion
  async function handleVote(side) {
    ...
    try {
      const tx =
        side === "tea"
          ? await program.rpc.votetea({
              accounts: {
                voteAccount: voteAccount.publicKey,
              },
            })
          : await program.rpc.votecoffee({
              accounts: {
                voteAccount: voteAccount.publicKey,
              },
            });
      ...
      <Box flex="1 0 auto">
        ...
        <Container>
          <Grid container spacing={3}>
            ...
            <Grid item xs={6}>
              <VoteOption side="tea" handleVote={handleVote} />
            </Grid>
            <Grid item xs={6}>
              <VoteOption side="coffee" handleVote={handleVote} />
            </Grid>
            <Grid item xs={12}>
